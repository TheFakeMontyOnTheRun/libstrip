...
public class Decal extends TriangleMesh {
	...
	private static float normalizeZ(byte z, HashMap<Byte, Float> zRoundings) {

		if (!zRoundings.containsKey(z)) {
			zRoundings.put(z, -((z + (-Byte.MIN_VALUE)) / 50.0f));
		}

		return zRoundings.get(z);
	}

	private static float normalizeX(byte x, HashMap<Byte, Float> xRoundings, float screenWidth, float screenHeight) {
	...
	private static float normalizeY(byte y, HashMap<Byte, Float> yRoundings, float screenWidth, float screenHeight) {
	...
	public static Decal loadDecal(String name, InputStream is, Direction d)
			throws IOException {
	...
	public static GeneralTriangle[] loadGraphic(InputStream is, float screenWidth, float screenHeight)
			throws IOException {
	...
	private static GeneralTriangle[] loadFrom(InputStream is, float offset, float screenWidth, float screenHeight) throws IOException {
		...
		HashMap<Byte, Float> xRoundings = new HashMap<>();
		HashMap<Byte, Float> yRoundings = new HashMap<>();
		HashMap<Byte, Float> zRoundings = new HashMap<>();

		ArrayList<GeneralTriangle> scratch = new ArrayList<>();
